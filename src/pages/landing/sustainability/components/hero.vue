    ...
    <div class="relative">
        ...
        <div class="absolute top-0 bottom-0 left-0 right-0 flex justify-center items-center px-5 lg:px-24">
            <div class="lg:flex lg:flex-col lg:gap-3 text-center">
                <span class="text-xl lg:text-5xl text-white-light font-semibold" :style='{ textShadow: "0px 1px 10px #B8B8B8" }'>{{language[1.1]}}</span>
                <span class="text-xl lg:text-5xl text-white-light font-semibold" :style='{ textShadow: "0px 1px 10px #B8B8B8" }'>{{language[1.2]}}</span>
            </div>
        </div>
    </div>
</template>
...
const language = landingLanguage[getLocalLanguage().key][RouterName.sustainability];
</script>