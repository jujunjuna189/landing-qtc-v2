        ...
        <table class="w-full border-collapse">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="border-y-[1.5px] border-white-muted px-3 py-2 text-sm font-semibold text-dark-muted text-start">Business</th>
              <th class="border-y-[1.5px] border-white-muted pl-3 pr-5 text-sm font-semibold text-dark-muted py-2"></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="product.length === 0" class="hover:bg-white-muted">
            ...
            <tr v-for="(item, index) in product" :key="index" class="hover:bg-white-muted">
              ...
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{{ item.business?.name }}</td>
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">
                <div class="flex gap-2 justify-end">
                  <UpdateModal :item="item" @onSave="getProduct" />
                  <ConfirmModal title="Confirm delete data" description="Are you sure you want to delete this data?" @onSave="(value) => onDeleteProduct(item.id)">
...
const product = ref<Array<ProductIF>>([]);

const getProduct = async () => {
  api.get('product')
    .then((res) => {
      product.value = res.list_data.data;
    })
    .catch((err) => {
      console.error(err);
    });
