<template>
    <DashboardLayout>
      ...
      <div class="flex justify-between">
        <span class="font-bold text-xl">News</span>
        ...
        <table class="w-full border-collapse">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="border-y-[1.5px] border-white-muted px-3 py-2 text-sm font-semibold text-dark-muted text-start w-[5rem]">Image</th>
              <th class="border-y-[1.5px] border-white-muted px-3 py-2 text-sm font-semibold text-dark-muted text-start">Title</th>
              <th class="border-y-[1.5px] border-white-muted px-3 py-2 text-sm font-semibold text-dark-muted text-start">date</th>
          ...
          <tbody>
            <tr v-if="news.length === 0" class="hover:bg-white-muted">
            ...
            <tr v-for="(item, index) in news" :key="index" class="hover:bg-white-muted">
              ...
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{{ item.date }}</td>
              ...
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">
                <div class="flex gap-2 justify-end">
                  ...
                  <ConfirmModal title="Confirm delete data" description="Are you sure you want to delete this data?" @onSave="(value) => onDeleteNews(item.id)">
...
const news = ref<Array<NewsIF>>([]);

const getNews = async () => {
  api.get('news')
    .then((res) => {
      news.value = res.list_data.data;
    })
    .catch((err) => {
      console.error(err);
    });
