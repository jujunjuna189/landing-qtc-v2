<template>
    <DashboardLayout>
      ...
      <div class="flex justify-between">
        <span class="font-bold text-xl">Assets</span>
        ...
        <table class="w-full border-collapse">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="border-y-[1.5px] border-white-muted px-3 py-2 text-sm font-semibold text-dark-muted text-start">File</th>
              <th class="border-y-[1.5px] border-white-muted pl-3 pr-5 text-sm font-semibold text-dark-muted py-2"></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="assets.length === 0" class="hover:bg-white-muted">
            ...
            <tr v-for="(item, index) in assets" :key="index" class="hover:bg-white-muted">
              ...
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{{ item.file }}</td>
              <td class="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">
                <div class="flex gap-2 justify-end">
                  <UpdateModal :item="item" @onSave="getAssets" />
                  <ConfirmModal title="Confirm delete data" description="Are you sure you want to delete this data?" @onSave="(value) => onDeleteAssets(item.id)">
...
const assets = ref<Array<AssetsIF>>([]);

const getAssets = async () => {
  api.get('asset-marketing')
    .then((res) => {
      assets.value = res.list_data.data;
    })
    .catch((err) => {
      console.error(err);
    });
...
const onDeleteAssets = (id: number) => {
  api.delete(`asset-marketing/${id}`).then((res) => {
    getAssets();
  }).catch((res) => {
      console.log(res);
  }).finally(() => {});
